            ...
            <View>
                {isPlayed && (
                    <View style={styles.playedOverlay}>
                        <View style={[styles.playedBackground, { backgroundColor: Color[`${color}100`] }]} />
                        <View style={[styles.playedBackgroundNeon, { backgroundColor: Color[`${color}500`] }]} />
                        <BlurView experimentalBlurMethod='dimezisBlurView' intensity={6} tint='default' style={styles.blurView} />
                    </View>
                )}
                ...
                {session.sessionType == 'RESERVED' ? (
                    ...
                    >
                        {session.reservationType == 'REGULAR' ? (
                            ...
                            <View style={styles.creatorInfo}>
                                ...
                                {session.creatorNickname && <Text style={styles.creatorNickname}>{session.creatorNickname}</Text>}
                            </View>
                        )}
                        ...
                        <Text numberOfLines={1} ellipsizeMode='tail' style={styles.sessionTitle}>{session.title}</Text>
                        ...
                        <View style={styles.sessionFooter}>
                            ...
                            {session.reservationType != 'EXTERNAL' ?
                                ...
                                :
                                <Text style={styles.externalUsage}>
                                    외부 인원 사용
                                </Text>
                            }
                        </View>
                    ...
                    <View key={session.sessionId} style={[styles.liveSessionCard, { width: width - 48 }]}>
                        <View style={styles.creatorInfo}>
                            <Text style={styles.creatorName}>{session.creatorName}</Text>
                            {session.creatorNickname && <Text style={styles.creatorNickname}>{session.creatorNickname}</Text>}
                        </View>
                        ...
                        <Text numberOfLines={1} ellipsizeMode='tail' style={[styles.sessionTitle, { color: isAfter ? Color['grey400'] : 'black' }]}>
                            {session.title}
                        </Text>
                        <View style={styles.sessionFooter}>
                            <Text style={styles.sessionTime}>{`${session.startTime.slice(0, -3)} ~ ${session.endTime.slice(0, -3)}`}</Text>
