 ...
 */
export function getReservationEditRequestBody(
  preReservation: ReservationForm & { reservationId: number },
  newReservation: ReservationForm,
): ReservationEditRequestBody & { reservationId: number } {
    ...
    if (
      ...
      if (!isEqual(oldValue, newValue)) {
        if (typedKey === "participators") {
          const oldParticipatorIds = (oldValue as Member[]).map(
            (item) => item.memberId,
          );
          const newParticipatorIds = (newValue as Member[]).map(
            (item) => item.memberId,
          );

          const added = newParticipatorIds.filter(
            (item: number) => !oldParticipatorIds.includes(item),
          );
          const removed = oldParticipatorIds.filter(
            (item: number) => !newParticipatorIds.includes(item),
          );

          if (added.length > 0) differences["addedParticipatorIds"] = added;
          if (removed.length > 0)
            differences["removedParticipatorIds"] = removed;
        } else if (typedKey === "borrowInstruments") {
          const oldParticipatorIds = (oldValue as Instrument[]).map(
            (item) => item.instrumentId,
          );
          const newParticipatorIds = (newValue as Instrument[]).map(
            (item) => item.instrumentId,
          );
          ...
          if (removed.length > 0)
            differences["removedBorrowInstrumentIds"] = removed;
        } else if (typedKey === "participationAvailable")
          differences[typedKey] = newValue as boolean;
        else if (typedKey === "reservationType")
          differences[typedKey] = newValue as Exclude<
            ReservationType,
            "EXTERNAL"
          >;
        else differences[typedKey] = newValue as string;
      }
    }
  });
