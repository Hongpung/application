...
export const ReservationCard: React.FC<ReservationCardProps> = ({
  ...
  const [startHour, startMinnute] = reservation.startTime
    .split(":")
    .map((time: string) => Number(time));
  const [endHour, endMinnute] = reservation.endTime
    .split(":")
    .map((time: string) => Number(time));
  ...
  const cardTopPosition =
    12 + (Number(startHour) - 10) * 80 + (startMinnute > 0 ? 40 : 0);
          ...
          {reservation.reservationType === "EXTERNAL" ? (
            ...
            >
              ...
              <Text style={style.footerText}>
                {reservation.amountOfParticipators}
              </Text>
            </View>
          )}
      ...
      {reservation.reservationType === "REGULAR" ? (
        ...
        >
          ...
          {reservation.creatorNickname && (
            <Text style={style.creatorNickname}>
              {reservation.creatorNickname}
            </Text>
          )}
        </View>
