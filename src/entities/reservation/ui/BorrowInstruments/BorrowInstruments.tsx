...
export const BorrowInstrumentsViewer: React.FC<BorrowInstrumentsViewerProps> = (
  props,
) => {
        ...
        >
          <View style={{ flexDirection: "row" }}>
            {instrumentTypes.map((type) => {
              ...
              if ("기타" === type) {
                const instCount = borrowInstruments.filter(
                  (instrument) =>
                ...
                return (
                  <View
                    key={`${type}-${instCount}`}
                    style={{ width: (width - 96) / 5, alignItems: "center" }}
                  >
                    <Text
                      style={{
