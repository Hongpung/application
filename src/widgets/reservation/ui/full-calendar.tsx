...
import { useMonthlyReservations } from "../lib/use-monthly-reservations"
...
export const FullCalendar: React.FC<FullCalendarProps> = ({ onClickDate, onError }) => {

    const today = new Date();

    const { calendarMonth, datesInMonth, incrementMonth, decrementMonth } = useCalendar();
    const { reservationsData, isLoading, isError } = useMonthlyReservations(calendarMonth);

    if (isError.isError) {
        Alert.alert('오류', isError.errorText, [{ text: '확인', onPress: () => onError(isError.errorText) }])
    }
