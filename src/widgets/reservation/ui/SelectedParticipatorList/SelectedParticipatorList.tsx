...
const SelectedParticipatorList: React.FC<SelectedParticipatorListProps> = ({ newParticipators, setNewParticipators }) => {
  return (
    <>
        ...
        renderItem={({ item: participator }) => (
          <View style={styles.participantContainer}>
            <Text style={[styles.participantText, { color: Color['grey500'] }]}>
              {participator.name}{participator.nickname ? `(${participator.nickname})` : ''}
            </Text>

            <Pressable
              onPress={() => {
                setNewParticipators(newParticipators.filter((participatorData) => participatorData.memberId != participator.memberId));
              }}>
