...
const ScheduleCardList =
    forwardRef<FlatList<ScheduleCard>, ScheduleCardListProps>((props, ref) => {

        const { isOnAir, scheduleCards, navigateToDetail, navigateToQRScan } = props

        const isNoSchedule = scheduleCards?.length == 1 && scheduleCards[0].type === 'empty';

        return (
            <View>
                {
                    isNoSchedule ?
                        <NoScheduleCard toNavigateQRScan={navigateToQRScan} />
                        :
                        <FlatList
                            ref={ref}
