...
// import { ClubInstrumentStackParamList } from '@hongpung/src/nav/InstrumentStack'
import { useLoadInstrumentDetailFetch } from '@hongpung/src/entities/instrument/api/instrumentApi'
import { ClubInstrumentStackParamList } from '@hongpung/legacy/nav/InstrumentStack'

const { width } = Dimensions.get('window')

type InstrumentDetailProps = NativeStackScreenProps<ClubInstrumentStackParamList, 'InstrumentDetail'>

const InstrumentDetailPage: React.FC<InstrumentDetailProps> = ({ navigation, route }) => {
    
    const { instrumentId } = route.params
    const { data, isLoading, error } = useLoadInstrumentDetailFetch({instrumentId})
        ...
        <View style={styles.container}>
            <Modal visible={imageModalVisible} transparent={true}>
                ...
                >
                    {data.imageUrl ? (
                        ...
                        <View style={[styles.modalImage, { backgroundColor: Color['grey200'] }]} />
                    )}
                </Pressable>
            </Modal>
            ...
            {loginUser?.club === data.club && loginUser?.role.length !== 0 ? (
                <Header
                    ...
                    HeaderName="악기 상세"
                    RightButton="수정"
                    RightAction={() => navigation.push('InstrumentEdit', { instrument: JSON.stringify(data) })}
                />
            ) : (
                <Header
                    leftButton="close"
                ...
                <View style={styles.infoContainer}>
                    ...
                    <View style={styles.infoRow}>
                        ...
                        <View style={styles.statusContainer}>
                            <Text style={[
                                styles.statusText,
                                { color: data.borrowAvailable ? Color['blue400'] : Color['red400'] }
                            ]}>
                            ...
                            data.borrowHistory.map(history => (
                                ...
                                >
                                    <View style={styles.historyHeader}>
                                        <Text style={styles.borrowerName}>{history.borrowerName}</Text>
