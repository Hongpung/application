...
import { ReservationForm } from "@hongpung/src/features/reservation/model/type";
...
import { reservationFormSubTitle } from "@hongpung/src/features/reservation/figureReservation/constant/reservationFormSubTitle";
import { instrumentTypes } from "@hongpung/src/entities/instrument/constant/instrumentTypes";

const EditReservationConfirmPage = () => {
    const { reservation, prevReservation, differentKeys, requestEditReservation } = useEditReservation();
    ...
    return (
        <View style={EditReservationConfirmStyles.container}>
            ...
            <ScrollView style={{ flexDirection: 'column', gap: 24, paddingVertical: 24 }}>
                ...
                <DifferenceData reservation={prevReservation} keys={differentKeys} textColor={"grey400"}
                />

                <Icons name={"arrow-down"} color={Color['blue500']} size={32} />

                <DifferenceData reservation={reservation} keys={differentKeys} textColor={"blue500"}
                />
...
const DifferenceData = ({ reservation, keys, textColor }: { reservation: ReservationForm; keys: (keyof ReservationForm)[]; textColor: keyof typeof Color }) => {
            ...
            {(() => {
                ...
                return keys.map((key) => {
                    switch (key) {
                        ...
                        case 'date':
                            return (
                                <View key={key} style={EditReservationConfirmStyles.rowItemContainer}>
                                    <Text style={EditReservationConfirmStyles.leftText}>{reservationFormSubTitle.date}</Text>
                                    <Text style={EditReservationConfirmStyles.rightText}>
                                        {reservation.date} ({daysOfWeek[new Date(reservation.date!).getDay()]})
                                    </Text>
                                </View>
                            );
                        case 'startTime':
                        case 'endTime':
                            ...
                            return (
                                <View key="time" style={EditReservationConfirmStyles.rowItemContainer}>
                                    <Text style={EditReservationConfirmStyles.leftText}>예약 시간</Text>
                                    <Text style={EditReservationConfirmStyles.rightText}>{`${reservation.startTime} ~ ${reservation.endTime}`}</Text>
                                </View>
                            );
                        case 'title':
                            return (
                                <View key={key} style={EditReservationConfirmStyles.rowItemContainer}>
                                    <Text style={EditReservationConfirmStyles.leftText}>예약명</Text>
                                    <Text style={EditReservationConfirmStyles.rightText}>
                                        {reservation.title.length > 0 ? reservation.title : `${loginUser?.nickname ? loginUser.nickname : loginUser?.name}의 연습`}
                                    </Text>
                                </View>
                            );
                        case 'participators':
                            return (
                                <View key={key} style={EditReservationConfirmStyles.rowItemContainer}>
                                    <Text style={EditReservationConfirmStyles.leftText}>참여자</Text>
                                    {reservation.participators.length > 0 ? (
                                        <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                                            <Text style={[EditReservationConfirmStyles.rightText, { maxWidth: 156 }]} numberOfLines={1}>
                        ...
                        case 'borrowInstruments':
                            return (
                                <View key={key} style={EditReservationConfirmStyles.rowItemContainer}>
                                    <Text style={EditReservationConfirmStyles.leftText}>대여 악기</Text>
                                    {reservation.borrowInstruments.length > 0 ? (
                                        ...
                                        <Text style={[EditReservationConfirmStyles.rightText, { color: Color['grey300'] }]}>없음</Text>
                                    )}
                                </View>
                            );
                        default:
