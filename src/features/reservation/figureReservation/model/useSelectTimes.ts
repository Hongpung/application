...
export const useSelectTimes = ({ occupiedTimes, startTime, endTime, selectedTimeBlocks, setTimeBlocks }:
    { occupiedTimes: TimeFormat[], startTime?: TimeFormat, endTime?: TimeFormat, selectedTimeBlocks: TimeFormat[], setTimeBlocks: (newBlocks: TimeFormat[]) => void }) => {
    ...
    const toggleTimeBlock = useCallback((time: TimeFormat) => {

        const sortedSelectedTimes = selectedTimeBlocks.sort((a, b) => TimeArray.indexOf(a) - TimeArray.indexOf(b)) ?? [];

        const timeIndex = TimeArray.indexOf(time);
        const firstSelectedTimeIndex = TimeArray.indexOf(sortedSelectedTimes[0]);
        const lastSelectedTimeIndex = TimeArray.indexOf(sortedSelectedTimes[sortedSelectedTimes.length - 1]);
        ...
        if (!sortedSelectedTimes.includes(time)) {
            if (sortedSelectedTimes.length === 0) {
                ...
                const containingOccupied = newTimes.filter(newTime => occupiedTimes.includes(newTime))
                ...
                if (containingOccupied.length > 0 && containingOccupied) {
                    if (firstSelectedTimeIndex !== startIndex) {
                        ...
                        const lastOccupiedIndex = TimeArray.indexOf(containingOccupied[containingOccupied.length - 1]) + 1;
                        const newTimes = TimeArray.slice(lastOccupiedIndex, endIndex + 1);
                        setTimeBlocks([...newTimes]);
                    } else if (lastSelectedTimeIndex !== endIndex) {
                        const firstOccupiedIndex = TimeArray.indexOf(containingOccupied[0]!) - 1;
            ...
            setTimeBlocks(sortedSelectedTimes.filter(selected => selected !== time));
        }
