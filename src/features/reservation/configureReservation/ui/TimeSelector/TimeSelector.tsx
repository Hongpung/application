...
export const TimeSelector: React.FC<TimeLineProps> = ({
  ...
  if (error || occupiedTimes == null) return null;

  return (
    <ScrollView ref={TimesRef}>
      {Array.from({ length: 13 }, (_, index) => index + 10).map(
        (time, index, array) => {
          return (
            <View key={"time-line" + index} pointerEvents="none">
              ...
              >
                <View>
                  <View
                    style={[
                      { position: "absolute", height: 9.2, width: 56, top: 0 },
                      selectedTimeBlocks.includes(
                        `${time - 1}:30` as TimeFormat,
                      )
                  ...
                  <View
                    style={[
                      { position: "absolute", height: 12, width: 56, top: 7.2 },
                      selectedTimeBlocks.includes(`${time}:00` as TimeFormat)
                        ? { backgroundColor: Color["blue100"], zIndex: 2 }
              ...
              {index < array.length - 1 && <View style={{ height: 56 }} />}
            </View>
          );
        },
      )}
          ...
          return (
            <Pressable
              ...
              style={[
                ...
                index !== 0 && { top: -index * 2, height: 42 },
                selectedTimeBlocks.includes(time) && {
              ...
              onPress={() => {
                if (!occupiedTimes.includes(time)) toggleTimeBlock(time);
              }}
            >
