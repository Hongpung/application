...
export const parseInstrumentCreateBody = async (
  instrumentCreateData: InstrumentCreateForm,
): Promise<InstrumentCreateBody> => {
  if (instrumentCreateData.instrumentType === null) {
    throw new Error("악기 종류를 선택해주세요.");
  }
  if (!!instrumentCreateData.selectedImage) {
    console.log("이미지 업로드 수행중");
    const uploadRespone = await uploadImageRequest(
      instrumentCreateData.selectedImage,
      "instruments",
    );
