    ...
    return (
        <View style={{
            ...
            <ScrollView contentContainerStyle={{
                ...
                <ParticipatorList participatorList={users}/>
            </ScrollView>
        </View>
    )
}
            ...
            <View style={{ flex: 1 }}>
                {participatorList.map((member) => (
                    <View key={member.name} style={{ marginVertical: 8, marginHorizontal: 24 }}>
                        <ProfileMiniCard user={member} isPicked={false} onPick={user => { setSelectedUser(user); }} view={'inClubView'} />
                    </View>
                ))}
            </View>

            <Modal transparent={true} visible={!!selectedUser}>
                <Pressable style={{
                    height: '100%',
