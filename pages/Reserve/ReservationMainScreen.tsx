            ...
            if (session.sessionType === 'RESERVED') {
                ...
                if (koreaTime < startTime) {                        //시간순에 따라 진행 전이 것이 그다음 출현
                    if (!slideTo.index) { //진행전 인것만 존재한다면 이전에 slideTo가 채워지지 않았으므로 따로 설정
                        slideTo.index = index
                        isOpen() && fetchReservationCards.push({ type: 'blank', nextReservationTime: session.startTime })
                ...
                if (!!sessionList[index + 1]) {
                    const timeGap: number = (TimetoDate(sessionList[index + 1].startTime).getTime() - TimetoDate(session.endTime).getTime()) / 1000 / 60
...
const SessionCardComponent: React.FC<{ session: Session }> = ({ session }) => {
    ...
    const color = session.sessionType == 'REALTIME' ?
        'red'
        :
...
const ReservationMainBottomLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    return (
        <View style={{ position: 'absolute', bottom: 92, paddingHorizontal: 24, height: 92, flexDirection: 'row', justifyContent: 'space-between', width: '100%' }}>
            {children}
        </View>
    )
}
