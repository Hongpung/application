...
const PracticeCard: React.FC<{ session: breifSessionInfo, onPress: (session: breifSessionInfo) => void }> = ({session, onPress }) => {
    ...
    return (
        <View style={{ marginHorizontal: 24, height: 120, borderRadius: 10, borderWidth: 1.5, borderColor: session.reservationType == '정규연습' ? isBefore ? Color['blue200'] : Color['blue500'] : session.participationAvailable ? isBefore ? Color['green200'] : Color['green500'] : isBefore ? Color['red200'] : Color['red500'] }}>
            <Text style={{
